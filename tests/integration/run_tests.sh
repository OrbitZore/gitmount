#!/usr/bin/env bash
# gitfs — integration tests (RFC 0000 §5).
...
#
# Usage: run_tests.sh [path-to-mount.gitfs]
set -u

HERE=$(cd "$(dirname "$0")" && pwd)
ROOT=$(cd "$HERE/../.." && pwd)
BIN=${1:-$ROOT/build/mount.gitfs}
export LC_ALL=C
...
mount_gitfs() {  # mount_gitfs <name> [extra options...]
  local name=$1; shift
...
check "cli: -t gitfs accepted" sh -c "'$BIN' '$REPO' '$TMP/x' -t gitfs -f >/dev/null 2>&1"
...
v1.0
v1.1" "$(ls -1 "$MNT/tag")"
  check_eq "annotated tag peels to commit" "hello gitfs" "$(cat "$MNT/tag/v1.0/README.md")"
...
main" "$(ls -1 "$MNT/remote/origin")"
  ...
  check_eq "upstream/HEAD content" "hello gitfs" "$(cat "$MNT/remote/upstream/HEAD/README.md")"
  ...
  MARKER="$MNT/branch/main/deps/libfoo.gitfs-submodule"
...
# ---------------------------------------------------------------------------
if mount_gitfs oversize -o blob-cache-size=1; then
...
if mount_gitfs empty; then
  ...
  check_eq "empty repo: json head null" "None" \
    "$(python3 -c 'import json; print(json.load(open("'$MNT'/.gitfs.json"))["head"])')"
  umount_gitfs
else
...
if [ "$(id -u)" = "0" ] && command -v mount >/dev/null; then
  ...
  if [ ! -e /usr/sbin/mount.gitfs ]; then
    cp "$BIN" /usr/sbin/mount.gitfs && HELPER_INSTALLED=1
  else
    cp "$BIN" /usr/sbin/mount.gitfs && HELPER_INSTALLED=0  # overwrite, keep
  fi
  trap 'fusermount3 -u "$TMP"/mnt-* 2>/dev/null; umount "$TMP"/mnt-m8 2>/dev/null; \
        [ "${HELPER_INSTALLED:-0}" = 1 ] && rm -f /usr/sbin/mount.gitfs; \
        cleanup' EXIT
  M8="$TMP/mnt-m8"; mkdir -p "$M8"
  M8ERR="$TMP/m8.err"
  if mount -t gitfs "$REPO" "$M8" 2>"$M8ERR"; then
    ...
    check "/proc/mounts type fuse.gitfs with ro,nosuid,nodev" \
      sh -c "grep '$M8 ' /proc/mounts | grep -q 'fuse.gitfs ro,nosuid,nodev'"
    umount "$M8" >/dev/null 2>&1 || fusermount3 -u "$M8"
    if mount --fake -t gitfs "$REPO" "$M8" >/dev/null 2>&1; then
    ...
    if mount -t gitfs -o noatime "$REPO" "$M8" >/dev/null 2>&1 && mountpoint -q "$M8"; then
      pass "mount(8) exec: -o noatime accepted"
      umount "$M8" >/dev/null 2>&1
    else
      fail "mount(8) exec: -o noatime"
    fi
    if mount -t gitfs -o noatime,nodiratime "$REPO" "$M8" >/dev/null 2>&1 && mountpoint -q "$M8"; then
      pass "mount(8) exec: -o noatime,nodiratime combo"
      umount "$M8" >/dev/null 2>&1
    else
      fail "mount(8) exec: -o noatime,nodiratime combo"
    fi
    if mount -t gitfs -o user=alice "$REPO" "$M8" >/dev/null 2>&1 && mountpoint -q "$M8"; then
      pass "mount(8) exec: -o user=<name> key-value form"
      umount "$M8" >/dev/null 2>&1
    else
      fail "mount(8) exec: -o user=<name>"
    fi
    if mount -t gitfs -o nouser "$REPO" "$M8" >/dev/null 2>&1 && mountpoint -q "$M8"; then
    ...
    skip "mount -t gitfs unavailable in this environment"
  fi
else
